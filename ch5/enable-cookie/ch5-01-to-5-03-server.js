...
/**
  ch5-01, ch5-02, 以及 ch5-03 的說明 請見 ./js/common.js
  ch5-01 和 ch5-03 的code 請見 ./js/common.js
  ch5-02 的code 請見本檔案的 第88, 90, 91行
*/
...
function setStatic(app) {
  ...
  app.use('/js', express.static(path.join(__dirname, 'js'))) // 使用同目錄的js
}

function setTemplate(app) {
  var templateDir = path.join(__dirname, '../../ch4/server/templates')
...
con.connect(function(err) {
  ...
  app.get('/', function (req, res) {
    console.log('req.cookies =', req.cookies) // 從瀏覽器發送過來的每個request都會自動將瀏覽器存的cookie帶到server端
    // "maxAge: 600000" 表示600000ms(毫秒)後過期, 請注意單位和網頁端的JavaScript不一樣
    res.cookie('from-server-hello', 'hello', { path: '/', maxAge:  600000 })
    res.cookie('from-server-world', 'world', { path: '/', maxAge: 1200000 })
